...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/05 15:46:57 by minkim3           #+#    #+#             */
/*   Updated: 2023/02/05 18:41:59 by minkim3          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	*(buffer + BUFFER_SIZE) = '\0';
	result = NULL;
	return_val = save_to_result(&buffer, &result, (save + fd));
	if (return_val == 1)
	{
		free(buffer);
		return (result);
		...
		return (exit_free(&buffer, &result, (save + fd)));
	return_val = read_line_continue(&buffer, &result, (save + fd), &fd);
